...
class DKService {
  ...
  static connect(List<String?> params, Function(Status status, dynamic error) statusCallback) async {
    _conn = await Nats.connect(opts: {
      'servers': [params[0]],
      'tls': params[2],
      // 'tls': TlsTrustedClient(Constants.env.clientCert, params[2]),
    }, authenticator: CredsAuthenticator(params[1]!), debug: true, statusCallback: statusCallback);
  }
  ...
  static Future<String> _request(String command, dynamic body, Map<String, dynamic> opts) async {
    ...
    dynamic data = await _conn!.request('$command', utf8.encode(body), opts);
